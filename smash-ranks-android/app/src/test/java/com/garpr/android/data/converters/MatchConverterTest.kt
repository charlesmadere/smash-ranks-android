...
class MatchConverterTest : BaseTest() {
    ...
    fun testFromJsonWithMatch0() {
        val match = matchAdapter.fromJson(JSON_MATCH_0)
        assertNotNull(match)
    }

    @Test
    fun testFromJsonWithMatch1() {
        val match = matchAdapter.fromJson(JSON_MATCH_1)
        assertNotNull(match)
    }

    @Test
    fun testFromJsonWithMatch2() {
        val match = matchAdapter.fromJson(JSON_MATCH_2)
        assertNotNull(match)
    }

    @Test
    fun testToJsonWithNull() {
        val json = matchAdapter.toJson(null)
        assertTrue(json.isNullOrEmpty())
    }

    @Test
    fun testToJsonWithMatch0() {
        val json = matchAdapter.toJson(MATCH_0)
        assertFalse(json.isNullOrBlank())

        val match = matchAdapter.fromJson(json)
        assertEquals(MATCH_0, match)
    }

    @Test
    fun testToJsonWithMatch1() {
        val json = matchAdapter.toJson(MATCH_1)
        assertFalse(json.isNullOrBlank())
    ...
    fun testToJsonWithMatch2() {
        val json = matchAdapter.toJson(MATCH_2)
        assertFalse(json.isNullOrBlank())
